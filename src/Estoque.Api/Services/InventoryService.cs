...
public sealed class InventoryService(StockDb db)
{
    ...
    public async Task<Order> Sell(NewOrder input)
    {
        Validate(input);
        await using var tx = await db.Database.BeginTransactionAsync();
        // Conditional UPDATE makes the stock check atomic, even for concurrent orders.
        var updated = await db.Products.Where(p => p.Id == input.ProductId && p.Stock >= input.Quantity)
            .ExecuteUpdateAsync(update => update.SetProperty(p => p.Stock, p => p.Stock - input.Quantity));
        if (updated == 0) throw new BusinessException("Produto inexistente ou estoque insuficiente.");
        var product = await db.Products.AsNoTracking().SingleAsync(p => p.Id == input.ProductId);
        var order = new Order { ProductId = product.Id, Quantity = input.Quantity, UnitPrice = product.Price, Customer = input.Customer.Trim() };
        db.Orders.Add(order);
